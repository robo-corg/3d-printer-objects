...
module PMS() {
    translate ([Thick*3-m*1, Width/2 - PMS_WIDTH/2, Thick]) {
        cube([PMS_LENGTH, PMS_WIDTH, 21.4]);
    }
}

module PMSFeet() {
    difference(){
        translate ([PMS_LENGTH, Width/2 - PMS_WIDTH/2 - Thick, Thick]) {
            cube([Thick*4, Thick*4, Thick*2]);
        }
        PMS();
    }
 
    difference(){
        translate ([PMS_LENGTH, Width/2 - PMS_WIDTH/2 - Thick*3 + PMS_WIDTH, Thick]) {
            cube([Thick*4, Thick*4, Thick*2]);
        }
        PMS();
    }
...
translate ([-m/2,0,0]){
    difference(){ 
        Panels();
        translate ([0, Width/2+1, Thick*2 + m]) {
            cube(20);
        }
        translate ([0, Width/2-25+4, Thick*2 + m + 0.5]) {
            cube([10, 16, 4]);
        }
        PMS();
    }
...
Coque();
        ...
        translate ([Thick*3, Width/2 - PMS_WIDTH/2, Thick]) {
        cube([PMS_LENGTH, PMS_WIDTH, Thick]);
    }

//addddPMS();
PMSFeet();
    
    translate([Length + 10,Width/2,2]){
        difference() {
...
color( Couleur1,1){
    translate([0,Width,Height+0.2]){
        rotate([0,180,180]){
                Coque();
            if (0) {
                                translate([Length - 10, Width/2+ 10, -0.5]) {
                    scale([-1, 1, 1]) {
                linear_extrude(height = 0.5){
                    text("AQ Monitor", font = Police, size = TxtSize,  valign ="center", halign ="left");
                    text("smcharg systems", font = Police, size = 4,  valign ="center", halign ="left");
